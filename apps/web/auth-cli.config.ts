...
// (src/modules/auth/auth.ts exports only the lazy `getAuth()` singleton, on
// purpose, so nothing connects to a database or an email provider at import
// time). This placeholder never opens the connection or sends an email, so
