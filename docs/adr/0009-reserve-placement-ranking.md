...
- **Then order by net real yield**: the position's yield (as % of CDI, converted with the 252-business-day convention) minus income tax, compared against the 12-month accumulated IPCA. Ties break by remaining FGC headroom for the holder's CPF in that financial conglomerate (R$ 250,000 per CPF per conglomerate, split among joint-account holders).
- **Tax bracket**: an existing position is taxed at the regressive-table bracket for its real holding age today, the number the household would pay if it redeemed now; a hypothetical new placement is taxed at the worst bracket (22.5%). LCI/LCA and poupança are exempt. The interpretation layer may explain that the bracket falls with time; the ranking does not anticipate it.
