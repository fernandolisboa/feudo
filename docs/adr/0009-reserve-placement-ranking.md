...
- **Then order by net real yield**: the position's rate, whatever its type (percentage of CDI, fixed annual, inflation-linked, the poupança rule, Tesouro Selic), converted to an annual nominal rate with the 252-business-day convention, minus income tax, compared against the 12-month accumulated IPCA. "As % of CDI" is one input form, not the comparison basis. Ties break by remaining FGC headroom for the holder's CPF in that financial conglomerate (R$ 250,000 per CPF per conglomerate; one holder hash per account, joint accounts are a later refinement if ever needed).
